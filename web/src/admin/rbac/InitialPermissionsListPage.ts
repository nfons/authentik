...
export class InitialPermissionsListPage extends TablePage<InitialPermissions> {
    ...
    async apiEndpoint(): Promise<PaginatedResponse<InitialPermissions>> {
        return new RbacApi(DEFAULT_CONFIG).rbacInitialPermissionsList(await this.defaultEndpointConfig());
    }
    ...
    row(item: InitialPermissions): TemplateResult[] {
        return [
            html`<a href="#/identity/initial-permissions/${item.pk}">${item.name}</a>`,
            html`<ak-forms-modal>
                <span slot="submit"> ${msg("Update")} </span>
                <span slot="header"> ${msg("Update Initial Permissions")} </span>
                <ak-initial-permissions-form slot="form" .instancePk=${item.pk}> </ak-initial-permissions-form>
